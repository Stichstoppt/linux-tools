...
# =============================================================================
# ULTIMATE WORKSPACE INITIALIZER (WITH TMUX & NVIM SETUP)
# =============================================================================

# 1. Eventuell alte tmux-Session ("dev") killen, um sauber zu starten
tmux kill-session -t dev > /dev/null 2>&1

# 2. Neue detached (Hintergrund) tmux-Session erstellen:
#    Name der Session: "dev", erstes Fenster heißt "nvim", startet in ~/ und führt nvim aus
tmux new-session -d -s dev -n nvim -c ~/ nvim

# 3. Das zweite Fenster in dieser Session erstellen:
#    Name: "bash", startet ebenfalls im Home-Verzeichnis (~/) mit deiner Standard-Shell
tmux new-window -t dev -n bash -c ~/

# 4. Sicherstellen, dass das Neovim-Fenster beim Start fokussiert ist
tmux select-window -t dev:nvim

# 5. Alle drei Apps parallel im Hintergrund anfeuern
#    Ghostty verbindet sich jetzt einfach blitzschnell mit unserer fertigen Session
...
# 6. Die Sortierschleife fängt die Fenster ab, sobald sie geladen sind
(
    for i in {1..12}; do
...
# 7. Standardmäßig direkt auf Workspace 2 schalten
hyprctl dispatch workspace 2 > /dev/null 2>&1

# Wichtig: füge Zeile " alias init-ws="~/init-ws.sh" " in .bashrc hinzu
